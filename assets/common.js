...
/* ---------- 每打死 1 隻怪要不要跳出來問繼續（2026-08-31 Steve：要有節奏感，且要能關掉）----------
   預設「ask」：每打死 1 隻（=1關）就跳出完整彈窗問要不要繼續。關掉（auto）就回到舊行為
   ——只顯示「過關」橫幅，自動接下一隻，只有 10 關全破才問。
   2026-09-05：從「每10題」改成「每10隻」（打死一隻怪才算過關，不再是答完幾題）。
   2026-09-06 Steve：「是每打死1隻怪物答對10題就問是否繼續，不是你現在這樣」——原本
   可以登入調整成每10~100隻才問一次（StageQ模組），跟「每擊敗1隻就問」的新設計直接
   衝突，索性拿掉那個可調整的批次大小，固定死每 1 隻就問，STAGE_QCOUNT 相關UI／模組
   一併移除（見 battle.html 拿掉 #stageqrow 那段）。標籤文字拿掉「10隻」的數字，
